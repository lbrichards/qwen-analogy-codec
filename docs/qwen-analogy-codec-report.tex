...
\]
holds with cosine similarity around 0.9996 under the full objective. A
classification-only ablation still recovers an analogy cosine of 0.9840,
suggesting that the relation is not created solely by the explicit analogy
term. The four word classes are also perfectly separated by a nearest-mean
classifier in the two-dimensional space, and the learned gender and royalty
directions are nearly orthogonal. The point of the experiment is not to claim a
universal semantic basis inside Qwen. It is to show, in a controlled setting,
that useful semantic structure can be pulled out of model activations and
compressed into a very small, inspectable interface. Held-out vocabulary tests
show partial generalization:
royal words such as \emph{prince} and \emph{duke} land nearest to \emph{king},
while \emph{princess} and \emph{duchess} land nearest to \emph{queen}. A
pseudoinverse hidden-state intervention gives small but directional evidence
for write access: increasing the 2D gender shift raises log-probabilities for
\emph{queen}, \emph{woman}, and \emph{princess}, while slightly lowering
\emph{king}.
\end{abstract}
...
template scheme:
\[
  \{\mathrm{prince}, \mathrm{princess}, \mathrm{boy}, \mathrm{girl},
  \mathrm{father}, \mathrm{mother}, \mathrm{duke}, \mathrm{duchess}\}.
\]
There was no retraining or refitting. Each held-out word was sampled in 50
contexts, projected through the existing encoder, and compared to the four
trained class means.
...
\bottomrule
\end{tabular}
\caption{Held-out vocabulary projected through the trained codec. Distances are
to the trained class means \(k=\mathrm{king}\), \(q=\mathrm{queen}\),
\(m=\mathrm{man}\), and \(w=\mathrm{woman}\).}
\label{tab:heldout-vocab}
\end{table}

\begin{figure}[h]
\centering
\includegraphics[width=0.92\linewidth]{heldout_vocabulary.pdf}
\caption{Trained classes are shown with filled markers; held-out classes are
shown with open markers. The royal masculine held-out words \emph{prince} and
\emph{duke} land nearest to \emph{king}, while \emph{princess} and
\emph{duchess} land nearest to \emph{queen}.}
\label{fig:heldout-vocab}
\end{figure}

The result is encouraging but not perfectly clean. The strongest pattern is in
the royal words: \emph{prince} and \emph{duke} land closest to the king mean,
while \emph{princess} and \emph{duchess} land closest to the queen mean. The
basic gender words also behave as expected: \emph{boy} is nearest to
\emph{man}, and \emph{girl} is nearest to \emph{woman}. The family terms are
messier: \emph{father} is pulled toward the king region and \emph{mother}
toward the queen region, suggesting that the current plane is not a pure
gender axis plus a pure royalty axis. Still, the held-out projection gives
evidence that the codec is not merely memorizing four training labels; it
transfers some structure to unseen but related vocabulary.
...
The previous sections treat the codec as a readout. A stronger question is
whether it can also support a small write operation. We therefore ran a simple
pseudoinverse intervention inspired by causal abstraction and model-editing
work \cite{geiger2021, meng2022, nanda2023}.

Let \(g_{2d} = \mu_{\mathrm{woman}} - \mu_{\mathrm{man}}\) be the learned
gender direction in the codec space. Starting at \(\mu_{\mathrm{king}}\), we
constructed targets
\[
  z_{\alpha} = \mu_{\mathrm{king}} + \alpha g_{2d},
  \quad \alpha \in \{0, 0.5, 1.0\}.
\]
Using the Moore--Penrose pseudoinverse of the encoder matrix, we mapped the 2D
shift back to a minimum-norm hidden-state perturbation:
\[
...
\end{quote}
and added \(\Delta x_{\alpha}\) to the hidden state of the \emph{king} token at
the codec layer before the later tokens attended to it. We measured continuation
log-probabilities for six target words.
...
\bottomrule
\end{tabular}
\caption{Continuation log-probability shifts relative to \(\alpha=0\).
The intervention raises queen/woman/princess continuations and slightly lowers
king/prince/man continuations.}
\label{tab:intervention}
\end{table}

\begin{figure}[h]
\centering
\includegraphics[width=0.86\linewidth]{intervention_logprob_shifts.pdf}
\caption{Log-probability shifts under the pseudoinverse hidden-state
intervention. The effect is small, but it is monotone and directionally aligned
with the intended 2D shift.}
\label{fig:intervention}
\end{figure}

This is not a dramatic steering result. The greedy completions were nearly
