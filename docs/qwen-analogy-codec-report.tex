...
A loss ablation shows that classification-only training still recovers a strong
analogy cosine of 0.9840, suggesting that the relation is not created solely by
the explicit analogy term. Held-out vocabulary tests show partial generalization:
...
\begin{tabular}{lrr}
\toprule
Continuation & Shift at \(\alpha=0.5\) & Shift at \(\alpha=1.0\) \\
\midrule
king     & -0.051 & -0.105 \\
queen    &  0.285 &  0.574 \\
prince   & -0.027 & -0.066 \\
...
\bottomrule
\end{tabular}
\caption{Continuation log-probability shifts relative to \(\alpha=0\), in nats.
